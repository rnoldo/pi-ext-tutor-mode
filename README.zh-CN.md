...
- **复习系统**：支持每日词汇复习 + 一题一题的互动测验（`tutor mode review`）
...
- `tutor-mode on`：开启学习模式
- `tutor-mode off`：关闭学习模式（专注编码）
- `tutor-mode status`：查看当前状态
- `tutor mode review`：在聊天中触发互动词汇测验（仅 Tutor Mode 开启时）

### 命令说明

- 当前仅支持 `tutor-mode on|off|status`
- 旧别名 `english-on` / `english-off` 已移除
